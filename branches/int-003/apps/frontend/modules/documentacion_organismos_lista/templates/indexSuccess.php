			...
			<tbody>
				<tr>
					<th width="10%" style="text-align:left;">
						<a href="<?php echo url_for('documentacion_organismos_lista/index?sort=fecha&type='.$sortType.'&page='.$paginaActual.'&orden=1&'.$redireccionGrupo) ?>">Fecha</a>
					</th>
					<th width="35%">
						<a href="<?php echo url_for('documentacion_organismos_lista/index?sort=nombre&type='.$sortType.'&page='.$paginaActual.'&orden=1&'.$redireccionGrupo) ?>">Titulo</a>
					</th>
					<th width="15%">
						<a href="<?php echo url_for('documentacion_organismos_lista/index?sort=organismo_id&type='.$sortType.'&page='.$paginaActual.'&orden=1&'.$redireccionGrupo) ?>">Organismo</a>
					</th>
					<th width="15%">
						<a href="<?php echo url_for('documentacion_organismos_lista/index?sort=user_id_creador&type='.$sortType.'&page='.$paginaActual.'&orden=1&'.$redireccionGrupo) ?>">Creado por</a>
					</th>
                                        <th>
				...
				<?php $i=0; foreach ($documentacion_organismo_list as $valor): $odd = fmod(++$i, 2) ? 'blanco' : 'gris' ?>
					<?php include_partial('ListaByOrganismo', array('valor'=>$valor,'odd'=>$odd, 'redireccionGrupo'=>$redireccionGrupo, 'responsable'=>$resposable));?>	
			        <?php endforeach; ?>
			</tbody>
		</table>
		<?php else : ?>
...
<!-- * -->
	<div class="rightside">
		<div class="paneles">
			<h1>Buscar</h1>
			<form method="post" enctype="multipart/form-data" action="<?php echo url_for('documentacion_organismos_lista/index?'.$redireccionGrupo) ?>">
