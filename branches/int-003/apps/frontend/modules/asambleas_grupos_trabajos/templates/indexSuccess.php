...
<div class="leftside"><!-- abre div left-side	-->
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="listados">
      ...
      <tr class="<?php echo $odd ?>">
        ...
        <td><a href="<?php echo url_for('asambleas_grupos_trabajos/ver?id=' . $convocatoria->getAsambleaId().'&'.$DAtos['get']) ?>"><strong><?php echo $convocatoria->getAsamblea()->getTitulo() ?></strong></a></td>
         <td><?php echo $convocatoria->getAsamblea()->getHorario() ?></td>      
        <td><?php echo $convocatoria->getAsamblea()->getDireccion() ?></td>
        <td><a href="<?php echo url_for('asambleas_grupos_trabajos/ver?id=' . $convocatoria->getAsambleaId().'&'.$DAtos['get']) ?>">M&aacute;s Informaci&oacute;n</a></td>
      </tr>
      <?php endforeach; ?>
    </table>
    <?php else: ?>
...
<!-- * -->
	<div class="rightside">
		<div class="paneles">
			<h1>Buscar</h1>
			<form method="post" enctype="multipart/form-data" action="<?php echo url_for('asambleas_grupos_trabajos/index') ?>">
				...
				<tbody>
					<tr>
						<td colspan="2" align="center" style="padding:8px;background-color:#EAEAEA;">
							...
							<br />
							<?php echo select_tag('tipo',options_for_select(array('0'=>'--seleccionar--','1'=>'Pendientes','2'=>'Aceptadas'), $sf_user->getAttribute('asambleas_grupos_trabajos_tipo')), array('style'=>'width:150px;margin-top:10px;'));?>
						</td>
					</tr>
					<tr><td style="height:5px;"></td></tr>
					<tr>
						<td>Titulo</td>
						<td><?php echo input_tag('titulo',$sf_user->getAttribute('asambleas_grupos_trabajos_nowtitulo'),array('class' =>'form_input', 'style'=>'width:190px;'));?></td>
					</tr>
					...
					<td>
							<?php echo select_tag('tipoasamblea',options_for_select(array('0'=>'--seleccionar--','anulada'=>'Anulada','caducada'=>'Caducada'),$sf_user->getAttribute('asambleas_grupos_trabajos_nowtipoasamblea')));?>
					</td>
					</tr>
					<tr>
					<td ><label>Fecha Desde:</label></td>
					<td width="71%" valign="middle">
						<input type="text" onblur="this.style.background='#E1F3F7'" onfocus="this.style.background='#D5F7FF'" style="width:80px;" name="desde_busqueda" id="desde_busqueda" value="<?php echo $sf_user->getAttribute('asambleas_grupos_trabajos_nowdesde')?>" class="form_input"/>
						<img border="0" style="margin-bottom: -3px;" src="/images/calendario.gif" class="clickeable" onclick="displayCalendar('desde_busqueda', this);"/>
                                        ...
                                        <tr>
                                                ...
                                                <td width="71%" valign="middle">
                                                        <input type="text" onblur="this.style.background='#E1F3F7'" onfocus="this.style.background='#D5F7FF'" style="width:80px;" name="hasta_busqueda" id="hasta_busqueda" value="<?php echo $sf_user->getAttribute('asambleas_grupos_trabajos_nowhasta')?>" class="form_input"/>
