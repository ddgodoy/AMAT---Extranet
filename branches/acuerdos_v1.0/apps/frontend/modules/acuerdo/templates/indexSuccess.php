...
?>
<link type="text/css" rel="stylesheet" href="/js/calendario/dhtml_calendar.css" media="screen"> 
...
<div class="mapa"><strong>Administraci&oacute;n</strong> > Acuerdos </div>
	<table width="100%" cellspacing="0" cellpadding="0" border="0">
		<tbody>
			<tr>
				<td width="70%"><h1>Acuerdos</h1></td>
				<td width="5%" align="center"><?php $nombretabla = 'Acuerdo'; echo link_to(image_tag('export_exel.jpg', array('title' => 'Exportar exel', 'alt' => 'Exportar exel', 'border' => '0')), 'inicio/exportar?tabla='.$nombretabla.'&filtro='.$sf_context->getModuleName().'_nowfilter&tipo=.xls'); ?></td>
	...
	<div class="leftside">
		<div class="lineaListados">
			...
			<span class="info" style="float: left;">Hay <?php echo $cantidadRegistros ?> Acuerdos</span>
			<?php if(validate_action('alta')):?>
			<input type="button" onclick="javascript:location.href='<?php echo url_for('acuerdo/nueva') ?>';" style="float: right;" value="Crear Nuevo Acuerdo" name="newNews" class="boton"/>
			<?php endif;?>
		</div>
		<?php if ($cantidadRegistros > 0) : ?>
		<form method="post" enctype="multipart/form-data" action="<?php echo url_for('acuerdo/delete') ?>">
		<table width="100%" cellspacing="0" cellpadding="0" border="0" class="listados">
			<tbody>
				...
				<?php $i=0; foreach ($Acuerdo as $valor): $odd = fmod(++$i, 2) ? 'blanco' : 'gris' ?>
				<tr class="<?php echo $odd ?>">
					...
					<td valign="top">
						<a <?php if(validate_action('listar')):?> href="<?php echo url_for('acuerdo/show?id=' . $valor->getId()) ?>" <?php endif; ?>>
							<strong><?php echo $valor->getNombre() ?></strong>
						</a>
					</td>
					<td valign="top" align="center">
					<?php if(validate_action('modificar')):?>
						<a href="<?php echo url_for('acuerdo/editar?id=' . $valor->getId()) ?>">
					...
					<?php endif;?>
                                      ...
                                      <td valign="top" align="center">
                                      <?php if(validate_action('baja')): ?>
                                            <?php echo link_to(image_tag('borrar.png', array('title'=>'Borrar','alt'=>'Borrar','width'=>'20','height'=>'20','border'=>'0')), 'acuerdo/delete?id='.$valor->getId(), array('method'=>'delete','confirm'=>'Confirma la eliminaci&oacute;n del registro?')) ?>
                                      <?php endif; ?>
                                      </td>
				</tr>
				<?php endforeach; ?>
		...
		<?php else : ?>
			...
			<?php else : ?>
				<div class="mensajeSistema comun">No hay acuerdo registradas</div>
			<?php endif; ?>
		<?php endif; ?>
		...
		<div class="lineaListados">
			...
			<span class="info" style="float: left;">Hay <?php echo $cantidadRegistros ?> Acuerdo/s</span>

		<?php if(validate_action('alta')):?>
		<input type="button" onclick="javascript:location.href='<?php echo url_for('acuerdo/nueva') ?>';" style="float:right;margin-top:10px;" value="Crear Nuevo Acuerdo" name="newNews" class="boton"/>
		<?php endif; ?>
		</div>
		<?php endif; ?>
	</div>
<!-- * -->
	<div class="rightside">
        <?php if ($acurdosCant >= 1) :
        echo include_partial('inicio/NavegacionGuiada',
        array('FEcha_circulares'=>$FEcha_circulares,
	   'modulo'=>$modulo,
	   'year'=>$year,
	   'months'=>$months,
	   'arrayCategoriasTema'=>CircularTable::doSelectAllCategorias('CategoriaAcuerdo'),
	   'SelectCatTemaBsq'=>$CatInicBsq,
	   'SelectSubTemaBsq'=>$SubIniBsq,
	   'SelectSubTemaBsq2'=>'',
	   'arrayCategoria'=>'',
	   'SelectCatOrganismoBsq'=>'',
	   'SelectSubOrganismoBsq'=>'')); endif; ?>
            
		<div class="paneles">
			<h1>Buscar por Titulo</h1>
			<form method="post" enctype="multipart/form-data" action="<?php echo url_for('acuerdo/index') ?>">
			<table width="100%" cellspacing="4" cellpadding="0" border="0">
				<tbody>
				    ...
				    <tr>
					<td valign="top"><label>Categor&iacute;a de acuerdo</label></td>
					<td valign="middle">
						<?php echo select_tag('select_cat_tema',options_for_select(array('0'=>'--seleccionar--')+_get_options_from_objects(CategoriaAcuerdoTable::getAll()), $CatInicBsq),array('style'=>'width:200px;','class'=>'form_input'));
							echo observe_field('select_cat_tema', array('update'=>'content_sub_tema','url'=>'acuerdo/subcategorias','with'=>"'id_categoria='+value")) ?>
					</td>
				</tr>
				<tr>
					<td valign="top"><label>Subcategor&iacute;a de acuerdo</label></td>
					<td valign="middle">
						<span id="content_sub_tema">
							<?php include_component('acuerdo','subcategorias',array('id_categoria'=>$CatInicBsq, 'id_subcategoria'=>$SubIniBsq));?>
						</span>
					</td>
				</tr>
					<tr>
