...
?>
<form action="<?php echo url_for('consejos_territoriales/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
	...
	<br />
	<fieldset>
		<legend>Consejo Territorial</legend>
	...
	<?php endif;?>	
		<input type="button" id="boton_cancel" class="boton" value="Cancelar" name="boton_cancel" onclick="document.location='<?php echo url_for('consejos_territoriales/index?page='.$pageActual) ?>';"/>
	</div>
</form>