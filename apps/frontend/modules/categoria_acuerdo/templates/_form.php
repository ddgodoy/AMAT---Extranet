...
<form action="<?php echo url_for('categoria_acuerdo/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<?php endif; ?>
    ...
    <fieldset>
      <legend>Categorías de acuerdos</legend>
    ...
    <?php endif;?>
      <input type="button" id="boton_cancel" class="boton" value="Cancelar" name="boton_cancel" onclick="document.location='<?php echo url_for('categoria_acuerdo/index') ?>';"/>
    </div>
</form>