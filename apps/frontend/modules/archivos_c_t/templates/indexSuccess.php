			...
			<tbody>
				<tr>
					...
					<th width="10%" style="text-align:left;">
						<a href="<?php echo url_for('archivos_c_t/index?sort=fecha&type='.$sortType.'&page='.$paginaActual.'&orden=1&'.$redireccionGrupo) ?>">Fecha</a>
					</th>
					<th width="35%">
						<a href="<?php echo url_for('archivos_c_t/index?sort=nombre&type='.$sortType.'&page='.$paginaActual.'&orden=1&'.$redireccionGrupo) ?>">Titulo</a>
					</th>
					<th width="15%">
						<a href="<?php echo url_for('archivos_c_t/index?sort=consejo_territorial_id&type='.$sortType.'&page='.$paginaActual.'&orden=1&'.$redireccionGrupo) ?>">Consejo territorial</a>
					</th>
					<th width="15%">
						<a href="<?php echo url_for('archivos_c_t/index?sort=owner_id&type='.$sortType.'&page='.$paginaActual.'&orden=1&'.$redireccionGrupo) ?>">Creado por</a>
					</th>
					<th width="5%">&nbsp;</th>
					<th width="5%">&nbsp;</th>
				</tr>
				...
				<tr class="<?php echo $odd ?>">
					...
					<?php if(validate_action('listar')):?>
						<a href="<?php echo url_for('archivos_c_t/show?id=' . $valor->getId().'&'.$redireccionGrupo) ?>">
					...
					<?php if(validate_action('modificar') || $valor->getOwnerId() == $sf_user->getAttribute('userId') ):?>
						<a href="<?php echo url_for('archivos_c_t/editar?id=' . $valor->getId().'&'.$redireccionGrupo) ?>">
					...
					<?php endif;?>	
                                          ...
                                          <?php if(validate_action('baja') || $sf_user->getAttribute('userId') == $valor->getOwnerId()):?>
                                                <?php echo link_to(image_tag('borrar.png', array('title'=>'Borrar','alt'=>'Borrar','width'=>'20','height'=>'20','border'=>'0')), 'archivos_c_t/delete?id='.$valor->getId().'&'.$redireccionGrupo, array('method'=>'delete','confirm'=>'Confirma la eliminaci&oacute;n del registro?')) ?>
                                          <?php endif;?>
                                          </td>
                                        </tr>
                                        <?php endforeach; ?>
...
<!-- * -->
	<div class="rightside">
		<div class="paneles">
			<h1>Buscar</h1>
			<form method="post" enctype="multipart/form-data" action="<?php echo url_for('archivos_c_t/index?'.$redireccionGrupo) ?>">
